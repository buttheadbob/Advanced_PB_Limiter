...
namespace Advanced_PB_Limiter.Utils
{
    public class NexusApiV3
    {
        ...
        public byte CurrentServerID { get; private set; } = new();
        public byte CurrentClusterID { get; private set; } = new();
        ...
        public NexusApiV3(Action? onEnabled = null)
        {
        ...
        {
            return Enabled && isPlayerOnline != null && (bool)isPlayerOnline.Invoke(playerId);
        }
        ...
        {
            if (Enabled)
                return (byte)(getTargetServer?.Invoke(position) ?? 255);
        ...
        {
            if (Enabled)
                return (int?)getTargetSector?.Invoke(position) ?? 255;
            return int.MinValue;
        }
        ...
        {
            return Enabled && (bool)(isServerOnline?.Invoke(serverID) ?? false);
        }
        ...
        {
            return Enabled && (bool)(sendModMsgToServer?.Invoke(MyTuple.Create(data, modChannelID, targetServer)) ?? false);
        }
        ...
        {
            return Enabled && (bool)(sendModMsgToAllServers?.Invoke(MyTuple.Create(data, modChannelID)) ?? false);
        }
        ...
        {
            if (Enabled)
                return (List<byte>?)getAllOnlineServers?.Invoke(getAllOnlineServers);
            return null;
        }
        ...
        {
            if (Enabled)
                return (List<ulong>?)getAllOnlinePlayers?.Invoke(null);
            return null;
        }
