...
namespace Advanced_PB_Limiter.Patches
{
    ...
    public static class ProfilerPatch_Torch
    {
        ...
        public static void Patch(PatchContext ctx)
        {
            ...
            ctx.GetPattern(_programmableRunSandboxed).Prefixes
                .Add(ReflectionUtils.StaticMethod(typeof(ProfilerPatch_Torch), nameof(PrefixProfilePb)));
            ctx.GetPattern(_programmableRunSandboxed).Suffixes
                .Add(ReflectionUtils.StaticMethod(typeof(ProfilerPatch_Torch), nameof(SuffixProfilePb)));
            ctx.GetPattern(_programmableRecompile).Prefixes
                .Add(ReflectionUtils.StaticMethod(typeof(ProfilerPatch_Torch), nameof(PrefixRecompilePb)));
            ctx.GetPattern(_programmableSaveMethod).Prefixes
                .Add(ReflectionUtils.StaticMethod(typeof(ProfilerPatch_Torch), nameof(SuffixSaveMethod)));
        ...
        private static FieldInfo? _runtimeField =
            typeof(MyProgrammableBlock).GetField("m_runtime", BindingFlags.Instance | BindingFlags.NonPublic);

        private static PropertyInfo? _updateFrequency = typeof(MyProgrammableBlock)
            .GetNestedType("RuntimeInfo", BindingFlags.NonPublic)
            .GetProperty("UpdateFrequency", BindingFlags.Public | BindingFlags.Instance);
        ...
        private static void SuffixProfilePb(MyProgrammableBlock? __instance, ref long __localTimingStart)
        {
            ...
            // Check the lowest Update type on the RuntimeInfo and divide run time accordingly.
            var __updateFrequency = (UpdateFrequency)(_updateFrequency?.GetValue(_runtimeField?.GetValue(__instance)) ??
            ...
            {
                if ((__updateFrequency & UpdateFrequency.Update1) != 0)
                    updateFrequency = UpdateFrequency.Update1;

                if ((__updateFrequency & UpdateFrequency.Update10) != 0)
                    updateFrequency = UpdateFrequency.Update10;

                if ((__updateFrequency & UpdateFrequency.Update100) != 0)
                    updateFrequency = UpdateFrequency.Update100;
            }

            var memoryUsage = Process.GetCurrentProcess().WorkingSet64 - beforeMemory;

            TrackingManager.UpdateTrackingData(__instance, runTime, memoryUsage, updateFrequency);

            if (Config.DebugReporting)
                Advanced_PB_Limiter.Log.Error(
                    $"{__instance.CustomName} : {measuredSpan.TotalMilliseconds / (Sync.ServerSimulationRatio > 1 ? 1 : Sync.ServerSimulationRatio) - overhead} :: {measuredSpan.TotalMilliseconds / (Sync.ServerSimulationRatio > 1 ? 1 : Sync.ServerSimulationRatio) - overhead}");
        }
        ...
        private static bool SuffixSaveMethod(MyProgrammableBlock? __instance)
        {
            ...
            if (Config.DebugReporting && player is not null)
                Log.Warn(
                    $"{player.DisplayName} ({__instance.OwnerId}) tried to save a programmable block while it was turned off.  This is not allowed.");
            return false;
        }
    }
}